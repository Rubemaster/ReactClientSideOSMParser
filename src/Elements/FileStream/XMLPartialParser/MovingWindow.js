...
export const MovingWindow = (putStash) => {
  ...
  let e = {
    ...
    tagProcessor: { putStash: putStash, action: ProcessTag }
  };
  while (e.at < testXML.length) {
    let action;
    let strike = false;
    for (
      let backTrack = 1;
      backTrack <= identifierMaxLength && backTrack <= testXML.length;
      backTrack++
    ) {
      if (e.at + backTrack < testXML.length) {
        const thisExtract = testXML.substring(e.at, e.at + backTrack);
        const iQ = IA[backTrack][thisExtract];
        if (iQ) {
          strike = true;
          action = iQ;
        }
